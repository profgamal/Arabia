...
function bindEvents() {
    ...
    // زر إعادة المسابقة في نتيجة المسابقة
...
// دالة تشفير النص باستخدام SHA-256
...
// كلمة المرور المشفرة (SN@2008++)
const HASHED_ADMIN_PASSWORD = '6c76bcf8ba92d5f08313c28d15bf34ee8c6ddb66e37527b5df12b4130d2ff31f';
...
function showAdminPanel() {
    ...
    form.onsubmit = async function(e) {
        ...
        if (isValid) {
            ...
            passwordInput.value = ''; // مسح كلمة المرور الخاطئة
            passwordInput.focus(); // إعادة التركيز على الحقل
        }
    };
}
...
function showFinalResult() {
    ...
    if (completedAllQuestions && Number(percentage) >= 100) {
        ...
        if (randomReward) {
            ...
            // تشغيل الألعاب النارية بعد ظهور البالونات مباشرة
            startFireworks();
        }
    ...
    resultHTML += `
        ...
        <div class="result-buttons">
            <button id="restart-btn" class="btn btn-primary">إعادة المسابقة</button>
    ...
    if (restartBtnEl) {
        restartBtnEl.addEventListener('click', function (e) {
            e.preventDefault();
            // إعادة تهيئة الحالة وإعادة البدء
            state.currentQuestionIndex = 0;
            state.score = 0;
            startQuiz();
        });
